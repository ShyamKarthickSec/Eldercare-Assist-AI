...
import { LuLayoutDashboard, LuHeart, LuMapPin, LuBell, LuLogOut, LuUser } from 'react-icons/lu';
...
const CaregiverSidebar = () => {
  ...
  useEffect(() => {
    ...
    const handleScroll = () => {
      const sections = ['overview', 'patient-status', 'location-status', 'alerts'];
  ...
  const navItems = [
    ...
    { id: 'patient-status', icon: LuHeart, label: 'Patient Status' },
    { id: 'location-status', icon: LuMapPin, label: 'Location' },
    { id: 'alerts', icon: LuBell, label: 'Alerts' },
  ];
  ...
  return (
    <aside className="patient-sidebar" style={{
      ...
      <div className="sidebar-header" style={{ borderBottom: '1px solid #e5e7eb', paddingBottom: '1rem', marginBottom: '1rem' }}>
        ...
        <p className="sidebar-subtitle" style={{ fontSize: '0.8rem', color: '#64748b', marginTop: '0.25rem' }}>Caregiver Portal</p>
        {userName && (
          <div style={{ 
            fontSize: '0.85rem', 
            color: '#475569', 
        ...
        {navItems.map(({ id, icon: Icon, label }) => (
          <button
            ...
            style={{
              ...
              borderRadius: '8px',
              border: 'none',
              backgroundColor: activeSection === id ? '#3b82f6' : 'transparent',
              ...
              boxShadow: activeSection === id ? '0 2px 8px rgba(59, 130, 246, 0.25)' : 'none'
            }}
            onMouseEnter={(e) => {
              if (activeSection !== id) {
